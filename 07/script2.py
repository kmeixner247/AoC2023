...
def compare_rank(hand1, hand2):
	hand1_value = evaluate_hand(optimize_joker(hand1))
	hand2_value = evaluate_hand(optimize_joker(hand2))
...
def optimize_joker(hand):
	uniques = set(hand)
	most_common = [None, 0]
	for card in uniques:
		if card != "J" and hand.count(card) > most_common[1]:
			most_common = [card, hand.count(card)]
	if most_common[0] == None:
		return "AAAAA"
	return hand.replace("J", most_common[0])
