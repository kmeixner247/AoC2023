# look at this marvel of efficiency, it only takes like an hour to run >.>
...
def find_unsupported_brick(bricks):
	for idx, brick in enumerate(bricks):
		if not is_brick_supported(brick, bricks[:idx]):
			return idx
	return -1
...
for i in range(len(bricks)):
	...
	bricks_copy.pop(i)
	first_unsupported = find_unsupported_brick(bricks_copy)

	while first_unsupported != -1:
		count += 1
		fall(bricks_copy, first_unsupported)
		first_unsupported = find_unsupported_brick(bricks_copy)
